...
export default function YogaTemplateAssign({
  ...
  const listUrl = useMemo(() => {
    ...
    return `${apiUrl.YOGA_TEMPLATES}?${params.toString()}`
  }, [page, perPage, search])

  const { data, isLoading, isFetching } = useQuery(
    ['yoga-template-assignment-list', listUrl],
    () => getData(listUrl),
    ...
    <>
      <div className="border rounded p-3 bg-white mb-3">
        <div className="flex flex-wrap items-center gap-3">
          <div className="text-sm font-semibold">Yoga Template</div>
