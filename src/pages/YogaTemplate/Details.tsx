...
import { getYogaTemplate } from './api'
...
export default function YogaTemplateDetails() {
  ...
  const loadTemplate = (page = daysPage, perPage = daysPerPage) => {
    if (!id) return
    getYogaTemplate(id, { page, per_page: perPage })
      .then((response: any) => {
        setTemplate(response?.yoga_template ?? response)
        setDaysMeta(response?.days_meta)
      })
      .catch(() => setError('Failed to load yoga template'))
  }
  useEffect(() => {
    setDaysPage(1)
  ...
  if (!template)
    return (
      <div className="p-6">
        <InfoBox content="Loading yoga template..." />
      </div>
    )
  ...
  return (
    <div className="p-4">
      <div className="flex items-center gap-2 mb-4">
        <button
          type="button"
          onClick={() => navigate('/yoga-templates')}
          aria-label="Back"
        >
          <Icons name="left-arrow-icon" />
        </button>
        <h1 className="text-xl font-semibold">
          Yoga Template Details - {template.name}
        </h1>
      </div>
      <TabContainer
        data={[
          { id: 'details', label: 'Details' },
          { id: 'days', label: 'Yoga Template' },
        ]}
        activeTab={activeTab}
        onClick={(item: any) =>
