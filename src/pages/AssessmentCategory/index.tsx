...
import { useAssessmentCategories, useDeleteAssessmentCategory } from './api'
...
const getTotalCount = (data: any, rowsLength: number) => {
  return (
...
export default function AssessmentCategory() {
  ...
  const { data, refetch, isFetching } = useAssessmentCategories(searchParams)
  const deleteMutation = useDeleteAssessmentCategory()
  const rows = useMemo(() => getRows(data), [data])
  const totalCount = getTotalCount(data, rows.length)

  useEffect(() => {
    setColumns(
      getAssessmentCategoryColumns({
        onNameClick: (row: any) => navigate(`/assessment-category/${row?.id}`),
      })
    )
  }, [navigate])
  ...
  const openView = (row: any) => {
    navigate(`/assessment-category/${row?.id}`)
  }
    ...
    <div>
      <ListingHeader
        ...
        actionProps={
          isNutritionist
            ? undefined
            : { actionTitle: 'Create Assessment Category' }
        }
        checkPermission={
          !isNutritionist && checkPermissions('ASSESSMENT_CATEGORY', 'create')
        }
      />
      <div className="p-4">
        <SmartTable
      ...
      <ConfirmDeleteModal
        ...
        title="Are you sure?"
        subTitle="Do you really want to delete this assessment category? This process cannot be undone."
        confirmLabel="Delete"
        cancelLabel="Cancel"
      />
    </div>
