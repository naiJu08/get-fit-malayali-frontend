  ...
  useEffect(() => {
    ...
    if (targetType === 'same_template') {
      if (sourceDays.length > 1) {
        ...
        setLoading(true)
        getData(
          `${apiUrl.WORKOUT_TEMPLATES}/${sourceTemplateId}?page=1&per_page=100`
        )
          .then((response: any) =>
            setTargets((response?.workout_template ?? response)?.days || [])
          )
          .catch(() => setError('Failed to load workout template days'))
  ...
  const clientTargetSummary =
    selectedClientTarget?.target_kind === 'legacy_plan'
      ...
      : selectedClientTarget?.target_kind === 'unassigned'
        ...
        : `Workout assignment: ${formatDate(selectedClientTarget?.assignment_start_date)} – ${formatDate(selectedClientTarget?.assignment_end_date)}`
