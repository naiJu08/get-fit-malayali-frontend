...
export default function WorkoutTemplateIndex() {
  ...
  const { data, isFetching, refetch } = useWorkoutTemplateList(params)
  const templates = data?.workout_templates ?? []

  const duplicate = async (row: any) => {
    try {
      await duplicateWorkoutTemplate(row.id)
      enqueueSnackbar('Workout template duplicated successfully', {
        variant: 'success',
      })
      refetch()
    } catch (error: any) {
  ...
  const remove = async () => {
    ...
    try {
      ...
      enqueueSnackbar('Workout template deleted successfully', {
        variant: 'success',
      })
      setDeleteId(null)
      refetch()
      ...
      renderCell: (row: any) => ({
        cell: (
          <button
            ...
            onClick={() => navigate(`/workout-templates/${row.id}`)}
          >
            {row.name}
          </button>
        ),
  ...
  return (
    <div>
      <ListingHeader
        data={{ title: 'Workout Templates', icon: 'workout' }}
        onActionClick={
