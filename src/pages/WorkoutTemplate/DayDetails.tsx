      ...
      {!loading && !error && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <DetailItem
              label="Template Id"
              value={safeStr(wp?.workout_template_id || wp?.template_id)}
            />
...
function AssignTabContent({
  ...
  const handleRemoveSelected = async () => {
    ...
    try {
      const res: any = await removeWorkoutTemplateExercises(
        wp.id,
        selectedExerciseIds
      )
...
export default function WorkoutPlanDetails() {
  ...
  const addExercisesAsync = ({ id, payload }: any) =>
    addWorkoutTemplateExercises(id, payload)
  const roleName = useAuthStore((s) => s.roleData?.name?.toLowerCase?.())
  const isNutritionist = roleName === 'nutritionist'
  ...
  const refreshDetails = useCallback(async () => {
    try {
      setLoading(true)
      const res = await getWorkoutTemplateDay(String(id))
      setData(res)
    } catch (e: any) {
    ...
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate(`/workout-templates/${templateId}`)}
            aria-label="Back"
          >
            <Icons name="left-arrow-icon" />
          </button>
          <h1 className="text-xl font-semibold">
            Workout Template Day - {wp?.title || `Day ${wp?.day_number}`}
          </h1>
        </div>
